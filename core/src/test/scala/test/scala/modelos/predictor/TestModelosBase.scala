...
import com.modelicious.in.tools.{Statistics, ScorerStats, StatsBase, SchemaTools}
import com.modelicious.in.transform._

import scala.reflect._

class TestModelosBase extends FlatSpec with Matchers with BeforeAndAfterAll {
    
  protected val master = "local"
  protected val appName = "test-spark"
  ...
  val raw_mock_DW = new DataWrapper( mock_RDD.map({ l => ( Array( l.label, l.features.toArray) ) })
   ...
   val mock_DW =  new DataWrapper( SchemaTools.convertToVectorOfFeatures(raw_mock_DW.data) )
  ...
  val a=SchemaTools.createCategoricMetadata( SchemaTools.convertToVectorOfFeatures(raw_mock_DW.data), 4 )
  val mock_DW=  new DataWrapper( SchemaTools.createCategoricMetadata( SchemaTools.convertToVectorOfFeatures(raw_mock_DW.data), 4 )._1 )
  
  }
  def getFeatureSelectedDW: DataWrapper = {
    

    val fs_conf =  <feature_selection>
	  						<method>mifs</method>
             		<num_features>2</num_features>
             		<num_partitions>2</num_partitions>
							</feature_selection>
    val fs = new FeatureSelectorTransform
    
    val fs_data = fs.exec(fs_conf, fixture.raw_mock_DW)
    new DataWrapper( SchemaTools.convertToVectorOfFeatures( fs_data.data ),  fs_data.getMetaData() )

  }
  
  def compareStats[T <: StatsBase]( a: T, b: T )( implicit ct : ClassTag[T] ) = {
    
    val cls = classTag[T].runtimeClass
    cls.getDeclaredFields.filter(_.getName != "Tiempo").foreach { f =>
      ...
      withClue(f.getName() + ": ") {
        if( java.lang.Double.isNaN(f.get(a).asInstanceOf[String].toDouble) ) {
          ...
          math.floor(f.get(a).asInstanceOf[String].toDouble*100)/100 should be (math.floor(f.get(b).asInstanceOf[String].toDouble*100)/100)
        }
      }
    }
  }
