...
private class NBScorer(modelo: NaiveBayesModel, discretizer: NBDiscretizer) extends Serializable {
  ...
  def predict(testData: RDD[LabeledPoint] ): RDD[(Double, Double)] = {
    testData.map { p =>
      ...
      val predicted = modelo.predictProbabilities(Vectors.dense(output_vector))
      //println( "NB Scored: "+ predicted.toArray.mkString("[",",","]") )
      (predicted(1), label)
    }
  }
}
...
/**
 * MLLIB Implementation of Naive Bayes Regressor
 * 
 * @see  [MLLIB NB Scorer](https://spark.apache.org/docs/1.6.0/mllib-naive-bayes.html)
*/
class NaiveBayesSC extends NaiveBayesBase with Scorer {

  override val name = "NaiveBayesSC"
  ...
  override def predict( testData: DataFrame ) : RDD[(Double, Double)] = {
    ...
    val my_RDD = SchemaTools.DFWithVectorToLabeledRDD(testData).cache()
    new NBScorer( modelo.getOrElse(throw new RuntimeException("Train you model before predict!")), discretizer).predict(my_RDD )
  }
